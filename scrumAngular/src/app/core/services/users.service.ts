...
export class UsersService {
    ...
    public async add(command: AddUserCommand) {
        ...
        return this.http.post(`${environment.apiUrl}/Users/v1/RegisterUser`, command).toPromise();
    }

    async login(username: string, password: string) {

        let loginResponse = await this.http.post<any>(`${environment.apiUrl}/Users/v1/Authenticate`, { username: username, password: password }).toPromise()
            .then(response => {
