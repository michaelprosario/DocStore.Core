...
import { Speaker } from '../core/entities/speaker';
import { SpeakerValidator } from '../core/validators/speaker.validator';
...
export class EditSpeakerComponent implements OnInit {
  ...
  record: Speaker;
  recordId: string;
  recordName: string = "speaker";
  recordName2: string = "Speaker";
  statusText: string;
  ...
  onClose() {
    this.router.navigate(['/list' + this.recordName2 + 's']);
  }

  private getDocFromRecord() {
    const doc = new Doc();
    doc.id = this.record.id;
    doc.collectionName = this.recordName2;
  ...
  formIsOkay() {
    const results = new SpeakerValidator().validate(this.record);
  ...
  onNew() {
    this.router.navigate(['/new' + this.recordName2]);
  }
}
